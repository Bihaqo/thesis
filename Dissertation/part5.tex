...
Тогда алгоритм вычисления $f$ может быть записан следующим образом
...
\end{algorithmic}
\end{algorithm}
%через последовательность шагов $k = 2, \ldots, d$
%\[
%r_k(\alpha_{k-1}^b,\alpha_{k-1}^A, \alpha_{k-1}^c) = \sum_{\alpha_{k-1}^b,\alpha_{k-1}^A, \alpha_{k-1}^c} r_{k-1}(\alpha_{k-1}^b,\alpha_{k-1}^A, \alpha_{k-1}^c) G_k^f(\alpha_{k-1}^b,\alpha_{k-1}^A, \alpha_{k-1}^c,\alpha_{k}^b,\alpha_{k}^A, \alpha_{k}^c) = \sum_{i_k, j_k, \alpha_{k-1}^b,\alpha_{k-1}^A, \alpha_{k-1}^c} G_k^b[i_k](\alpha_{k-1}^b, \alpha_{k}^b) G_k^A[i_k, j_k] \otimes G_k^c[j_k]
%\]
...
\end{tabular}
&
% \includegraphics[width=1.8cm]{figures/Z_legend_worked3.pdf}
\\
(a)
&
(b)
...
% \vspace{-0.1cm} First we compare our method (TT) against the following methods implemeted in the LibDAI system~\cite{mooij10libdai}: Belief Propagation (BP)~\cite{kschischang01bp}, Tree Expectation Propagation (TREEEP)~\cite{minka04treeep}, and the Mean Field method (MF)~\cite{wainwright08gm}. In addition we compare against the annealed importance sampling method (AIS)~\cite{neal01ais} as a representative of the family of MCMC methods. The results are shown in plot~\ref{fig:zExperiments}a. For AIS method we've selected parameters (1000 intermidiate distribution, 70 samples each) to maximize the accuracy achieved within 60 seconds per model. The groundtruth values are computed using the junction tree algorithm. We report the average value ($\pm$ one standard deviation) of the absolute error of the logarithm of the computed partition functions.
...
% \vspace{-0.1cm} In the second experiment we compare our method with the recently proposed WISH method~\cite{ermon13wish} on a dataset taken form their paper. The results of the comparison are presented in plot~\ref{fig:zExperiments}b. Note, that the WISH system is computationally very heavy and for this particular dataset it was run on a computational cluster for at least 15 minutes on each core. Our non-optimized MATLAB code worked at most 60 seconds on each example.
...
% \vspace{-0.1cm} Finally, fig.~\ref{fig:zConf} presents the confidence bounds obtained by upper-bounding the result of theorem~\ref{main-theorem} using eq.~\eqref{eq:l2computing} and taking the logarithm of the inequality.\\[-0.8cm]
